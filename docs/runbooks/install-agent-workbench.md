...
the `.tgz` from that page and `npm install -g ./auriora-agent-workbench-0.6.3.tgz`.

The commands above install release `0.6.3`, including the compiled lightweight
...
claude plugin list   # -> agent-workbench@agent-workbench-local  v0.6.3  enabled
```
...
codex plugin list   # -> agent-workbench@agent-workbench-local  v0.6.3  installed, enabled
```
