...
| 4 | Ruby/Rails | current `partial_semantic` | Specs 047 and 048 plus dogfood follow-up deliver catalog/project-shape/context/validation planning and one `tree-sitter-ruby` path for declarations, bounded references, conservative impact, static namespace and keyword/hash-rocket route targets, resource controllers, and bounded model/concern DSL relationships; runtime dispatch and metaprogramming remain deferred |
