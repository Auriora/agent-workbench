...
| 5 | CloudFormation/SAM | `resource_backed`, then `partial_semantic` | YAML/JSON template traversal, logical resources, Lambda handler/file routing, explicit SAM event sources, intrinsic/dependency edges with low-confidence provenance, policy-aware non-executed validation planning, deeper stack semantics deferred |
