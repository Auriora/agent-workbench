...
| 4 | Ruby/Rails | current `partial_semantic` | Specs 047–049 plus dogfood follow-up deliver catalog/project-shape/context/validation planning and one `tree-sitter-ruby` path for declarations, canonical singleton methods, bounded references, conservative impact, distinct path/controller-module route scopes, static resource/custom-action/draw targets, and ambiguity-gated association relationships including HABTM and `source_type`; runtime dispatch and metaprogramming remain deferred |
