...
| ranked documentation readiness | Prove one published snapshot drives first-read trust and ranked search | same-snapshot concern state, exhaustive recovery categories, status/orientation/search agreement, and two installed clients |
...
| ranked documentation readiness | graph-store, status, orientation, docs-search, and daemon fixtures | exact selected snapshot, ready/invalid/unavailable state, recovery category, bounded reason, reusable-or-blocked orientation, callable status action | status, orientation, and initial search agree; only refreshable evidence enters the coordinator; no-map remains usable |
| production documentation corpus and owner priority | containing-product and selected-fixture-root docs fixtures plus ranking/store/MCP tests | `production-docs-v1` root relativity, no excluded-content read, discovered/eligible/excluded conservation, excluded-owner evidence, stale-policy refresh receipt, exact SessionStart owner-first order, v1 universe invalidation | containing product excludes embedded fixture Markdown; selected fixture retains it; current canonical owner ranks first; all public counts describe the searched corpus |
...
| installed ranked-search parity | real repository daemon plus real Codex and Claude Code CLIs | identical SessionStart-governance request, shared published snapshot, ready status, reusable orientation, complete ranked universe, canonical/current governing design | both clients select the same published snapshot and agree on runtime/plugin version, readiness, trust, first canonical path, authority, status, and concern evidence; dated snapshot identities belong in the dogfood ledger |
...
| public MCP failure presentation | `fixture-redaction-boundary` plus shared envelope, resource, diagnostics, docs, graph, workspace-edit, orientation/status/scope/overview, and integration-health contracts | hostile Unix and Windows paths, workspace escape, secret assignment, private key, safe phrase retention, marker-only fallback, idempotence, multibyte 512-byte bound, and duplicated public fields | serialized failures omit hostile sentinels while preserving error code, retryability, cause-code-dependent data, metadata, trust, and next actions |
...
| `verification_plan` skipped-path compaction | generated-heavy Agent Workbench validation fixture plus scanner/context/presenter contracts | exact conserved reason groups after more than 100 skips, lexical three-path samples, independent source truncation, bounded actionable exclusions, redaction, no raw current output, and five repository gates exactly once | payload compaction preserves typecheck, test, plugin validation, skill validation, and package dry-run without introducing a scan or extraction limit; task context reports the same population groups |
...
| documentation concern extraction | checked-in map plus focused owner fixtures | production extractor output, exact invalid-owner reason, containment-checked 120,000-byte docs read and 16,384-byte classification boundaries | checked-in map returns `complete`; directory owners fail; valid large owners are classified without materializing their body; missing bounded-read capability and oversized or unterminated metadata fail truthfully; published byte count/truncation remain exact |
| documentation publication performance | real-repository rebuild plus graph-store/daemon integration | no node-FTS cleanup on first file insertion, scoped cleanup on true replacement, unchanged finite worker deadline, atomic concern-index publication | refresh publishes a ranking-ready snapshot; regression observes zero cleanup statements for first insertion and one for replacement; duplicate node-FTS identity remains EB062 |
