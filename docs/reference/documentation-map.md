...
| FTS-backed docs search and currency routing | [MCP surface design](../design/mcp-surface-design.md) | docs search; documentation intent; documentation ranking; documentation corpus; count semantics | Owns the production documentation corpus, exact concern resolution, governing-owner-first ranking, bounded FTS-plus-owner union, frozen pagination, recovery, direct-read, and lifecycle behavior. Schema-v3 concern/ranked-universe storage and count retrieval live in [Graph store design](../design/graph-store-design.md); public corpus, hit, rank, count/filter, cursor, failure, trust, and compatibility fields live in [Runtime contracts](runtime-contracts.md). |
...
| Cross-platform packaging delivery record | [Spec closure log](../history/spec-closure-log.md) |  | Closed Spec 033 delivery evidence. Current npm install and Windows portable-distribution contracts, platform matrix, plugin launch, hooks, and native dependency guidance live in [Install Agent Workbench](../runbooks/install-agent-workbench.md), [Codex Agent Workbench plugin runbook](../runbooks/codex-agent-workbench-plugin.md), [packaging README](../../packaging/agent-workbench/README.md), and [Native dependency setup](../runbooks/native-dependency-setup.md). Active Spec 063 owns delivery of the Windows x64 portable asset under EB047. |
