...
| FTS-backed docs search and currency routing | [MCP surface design](../design/mcp-surface-design.md) | Current `docs_search`, `docs_current_for_task`, docs inventory currency labels, ranking caveats, direct-read requirements, and lifecycle boundary behavior. Storage/index details live in [Graph store design](../design/graph-store-design.md); public fields live in [Runtime contracts](runtime-contracts.md). |
