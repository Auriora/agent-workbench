...
| REQ-032 | The public `changed_files_context` tool must discover bounded staged, unstaged, and untracked Git paths and join repository status, diagnostics, validation planning, and optional observational lifecycle context into explicit ready, no-change, degraded, or blocked component states without mutation or command execution. | Workflow service, MCP surface, agent integration | Edit and validation design, runtime contracts, lifecycle bridge contract | Git adapter, use-case, MCP registry, integration, and packaged guidance tests |
