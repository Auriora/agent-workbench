...
| --- | --- | --- | --- | --- |
| 13 / 49 | partial | Remaining result-budget truncation after raising normal runtime catalog scan budgets. The latest full eight-repo report had docs list/section partials in TimeLocker, aws-datalake, and OneMount plus one CrealityPrint reference truncation; focused subset evidence showed requested docs section partials removed. | Implemented for the known roots: docs overview/map now have cursor-backed pagination and cursor-backed truncation is classified as a complete page; `find_references` now distinguishes real omitted result rows from catalog metadata sampling. | Rerun a reliable full multi-repo warmup sweep after the PTY/report issue is fixed; then address any remaining non-full rows from that fresh report. |
...
| 2 / 2 | blocked | `docs_search` returned blocked cold/missing FTS evidence when no markdown docs were indexed into the graph/docs FTS snapshot for FreeCAD and LibreChat under bounded graph warmup. | Implemented: graph warmup now scans enough files for docs FTS while bounding symbol extraction separately. Direct checks showed usable docs FTS and non-blocked docs search metadata for both repos. | Confirm in a fresh full multi-repo warmup sweep once the stale PTY/report issue is isolated. |
