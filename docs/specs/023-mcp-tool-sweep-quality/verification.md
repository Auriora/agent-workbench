...
| --- | --- | --- | --- | --- |
| 13 / 49 | partial | Remaining result-budget truncation after raising normal runtime catalog scan budgets. The latest full eight-repo report had docs list/section partials in TimeLocker, aws-datalake, and OneMount plus one CrealityPrint reference truncation; focused subset evidence now shows requested docs section partials removed, leaving only `docs-overview` and `docs-map` broad list caps in those three repos. | Implementation debt. These should not be accepted as normal; the runtime needs a broad docs-list budget policy and a clear reference-result budget policy. | Add docs overview/map pagination or summary semantics that do not label useful bounded lists as unexplained partial results; tighten reference pagination or result summaries. |
...
| 2 / 2 | blocked | `docs_search` returns blocked cold/missing FTS evidence when no markdown docs are indexed into the graph/docs FTS snapshot for FreeCAD and LibreChat under bounded graph warmup. | Runtime warmup gap. Missing/cold docs FTS is now correctly blocked, not invalid, but it still needs root-cause work. | Ensure graph warmup indexes docs independently of low symbol budgets or add a targeted docs FTS warmup path. |
