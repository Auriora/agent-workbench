...
| --- | --- | --- | --- | --- |
| 13 / 49 | partial | Remaining result-budget truncation after raising normal runtime catalog scan budgets. TimeLocker, aws-datalake, and OneMount still hit docs list/map budgets that are inherited by outline and read-section calls; CrealityPrint still truncates one reference result. | Implementation debt. These should not be accepted as normal; the runtime needs targeted document lookup for requested files/sections and a clear reference-result budget policy. | Add targeted docs lookup that does not depend on broad docs-list completeness; tighten reference pagination or result summaries instead of returning unexplained partial results. |
