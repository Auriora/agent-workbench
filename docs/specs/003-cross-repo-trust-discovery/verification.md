...
| 2026-06-05 | AWS resource reliability and query metadata | `pnpm typecheck`; `pnpm exec vitest run tests/graph/query-tools.test.ts tests/graph/extraction-pipeline.test.ts tests/docs/docs-links-metadata.test.ts`; `pnpm test`; `pnpm debug:sample-smoke -- --repo /home/bcherrington/Projects/Clients/Co-foundry/aws-datalake --max 1 --context`; composed-server resource smoke against `aws-datalake` with startup warmup enabled | Passed focused tests, full suite with 38 files and 238 tests, and AWS read-only smoke. Local status/scope/overview/context completed in 324 ms before the fix and passed again after the fix. Composed resource reads returned while warmup was enabled: `repo:///status` 14 ms, `repo:///scope` 183 ms, `repo:///overview` 183 ms, `integration:///profiles/codex` 11 ms. Added cooperative graph-index yields and deferred startup warmup one event-loop turn; `symbol_search` metadata now includes languages from returned symbols. |
| 2026-06-05 | .NET comparison feedback | Read `docs/reference/dotnet/modena-aec-dotnet-evaluation-2026-06-05.md` | Accepted as durable comparison artifact. Roadmap updated for .NET generated-output handling, `.sln`/`.csproj` project graph extraction, C#/Razor routing anchors, `dotnet build`/`dotnet test` validation planning, and fixture-backed C#/Razor partial semantic support. |
