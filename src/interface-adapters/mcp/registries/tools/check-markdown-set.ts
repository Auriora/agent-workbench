...
const checkMarkdownSetRawShape = {
  ...
  cursor: z.string().min(1).optional().describe("Opaque continuation cursor returned by a previous check_markdown_set call with the same scope and options."),
  exclude_active_specs: z.boolean().default(false).describe("Exclude scope-discovered docs/specs packages while retaining explicitly requested spec paths."),
...
const checkMarkdownSetDescription = "Use this before docs-wide commits, spec closure, or documentation promotion to audit explicit Markdown files or a large docs scope through deterministic cursor chunks. It reports measurable document coverage, can exclude active spec packages by intent, and returns aggregate bounded findings without mutation.";
...
export const checkMarkdownSetTool: McpToolDeclaration = {
  ...
  metadata: {
    ...
    mutation_class: "none",
    budget_policy: "Scans Markdown inventory once per call and is bounded by explicit paths or scope_path, cursor, max_documents, max_findings, file-size, and evidence limits.",
    ...
    parameters: [
      ...
      { name: "cursor", description: "Opaque cursor for the next deterministic chunk of the same audit.", required: false },
      { name: "exclude_active_specs", description: "Exclude scope-discovered active spec package documents; explicit paths remain included.", required: false },
